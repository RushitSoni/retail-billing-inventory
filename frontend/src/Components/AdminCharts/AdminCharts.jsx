...
const AdminCharts = (props) => {
  ...
  useEffect(() => {
    if (bills.length || customers.length || inventory.length || shops.length) {
      ...
      <div className="chart-container">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart layout="vertical" data={topShopsData}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis type="number" />
            <YAxis dataKey="shop" type="category" tick={{ angle: 0 }} width={100} tickFormatter={(value) => value.length > 15 ? `${value.substring(0, 15)}...` : value} />
            <Tooltip />
            <Legend />
            <Bar dataKey="bills" fill="#FFBB28" />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
